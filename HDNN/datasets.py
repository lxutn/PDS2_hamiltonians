...
STL10_MEAN_VALS = [0.4467, 0.4398, 0.4066]  #mean value of the image for each color
STL10_STD_VALS = [0.2603, 0.2566, 0.2713]   #std value of the image for each color
...
def CIFAR_dataset(dataset, path, batch_size, crop_and_flip, kwargs):
    ...
    preprocess_train = transforms.Compose([
        transforms.ToTensor(),                                     
        transforms.RandomCrop(CIFAR_SIZE, padding=NB_PIXEL_PADDING_CIFAR, fill=PADDED_VALUE),
...
def STL10_dataset(path, batch_size, crop_and_flip, kwargs):
    ...
    preprocess_train = transforms.Compose([
        transforms.ToTensor(),                                     
        transforms.RandomCrop(STL10_SIZE, padding=NB_PIXEL_PADDING_STL10, fill=PADDED_VALUE),
        transforms.RandomHorizontalFlip(p=PROBA_FLIP),
    ...
    #Create the data loaders
    train_loader = torch.utils.data.DataLoader(
                        datasets.STL10('../data', split="train", download=True, transform=preprocess_train),
