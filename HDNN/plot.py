...
for i in range(len(param1_values)):
    for j in range(len(param2_values)):
        for it in range(STATS_REPETS):
            ...
            nb_epochs = len(train_acc_e[i][j][it])
...
if len(param1_values) == 1 and len(param2_values) == 1:
    ...
    for col_name in COL_NAMES:
        print(col_name+": "+str(df[col_name].mean()) +
              " with std "+str(df[col_name].std()))
    # Line plot for accuracy and loss
    for measure in ["accuracy", "loss"]:
        plt.figure()
        plt.title(DATASET+": "+measure+" along epoch with " +
...
# 2D grid
    ...
    for col_name in COL_NAMES:
        ...
        x = df.pivot_table(
            index=params_names[0], columns=params_names[1], values=col_name, aggfunc=np.mean)
        ...
        plt.title(DATASET+": "+col_name+" std w.r.t. "+str(params_names[0])+" and "+str(params_names[1])
                  + " with "+str(STATS_REPETS)+" iter.")
        x = df.pivot_table(
            index=params_names[0], columns=params_names[1], values=col_name, aggfunc=np.std)
...
# 1D grid
    ...
    for i, col_name in enumerate(COL_NAMES):
        ...
        sns.lineplot(data=df, x=param_name,
